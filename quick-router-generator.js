...
let root = path.resolve(__dirname, "src/pages");
console.log("开始配置>>>>>>>>>>>>>>>>>>\n");
// 读取文件信息，并生成路由
function readFileInfo(pathName, dirName) {
  ...
  if (dirDetails.includes("config.json")) {
    ...
    const json = JSON.parse(
      fs.readFileSync(path.resolve(pathName, "config.json"))
    );
    // 判断是否有exclude属性
    ...
    if (child && child.length > 0) {
      ...
      childList = ObjArrDuplicate(child).map((element) => {
        if (!element.parent) {
          throw new Error(
            "parent属性不能为空 " + path.resolve(pathName, "config.json")
          );
        }
  ...
  dirDetails.forEach((item) => {
    ...
    if (is.isFile()) {
      ...
      if (suffix == ".vue") {
        ...
        routes.push(`
                {
                    name:'/pages${routeName ? routeName + "-" : ""}${fileName}',
                    path:'/pages${dirName ? "/" + dirName : ""}/${fileName}',
                    component:() => import('@/pages${
                      dirName ? "/" + dirName : ""
                    }${fileName != "index" ? "/" + fileName + ".vue" : ""}')
                }`);
...
function ObjArrDuplicate(array) {
  let list = ArrayDuplicate(array.map((item) => JSON.stringify(item))).map(
...
fs.writeFile(
  ...
  toStringFormat(readFileInfo(root, "")),
  (err) => {
    if (err) return;
    console.log("配置成功 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
  }
